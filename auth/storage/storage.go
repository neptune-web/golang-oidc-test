...
// for simplicity keep everything in-memory right now
...
func NewStorage() *Storage {
	...
	return &Storage{
		...
		clients:       clients,
		services: map[string]Service{
			// this is used in example client, should change to our own service name
			"sdc": {
				keys: map[string]*rsa.PublicKey{
					"key1": serviceKey1,
				},
...
func (s *Storage) CheckUsernamePassword(username, password, id string) error {
	s.lock.Lock()
	defer s.lock.Unlock()
	// find the corresponding auth request in the storage
	request, ok := s.authRequests[id]
	if !ok {
		return fmt.Errorf("request not found")
	}

	// Use LDAP to check user credentials
	// TODO: It's not a great idea to reconnect for every request, would be better to maintain the connection
	// with permanent connection this func would need to rebind with read-only user at the end.
	l, err := ldap.DialURL(config.Get("LDAP_URI"))
	if err != nil {
		return err
	}
	defer l.Close()
	// Bind with a read-only user
	err = l.Bind(config.Get("LDAP_BIND_DN"), config.Get("LDAP_BIND_PASSWORD"))
	if err != nil {
		return err
	}
	...
	searchRequest := ldap.NewSearchRequest(
		username, // should be replaced with proper search based on the CID (workforceid field)
	...
	// TODO: remove this later, it's just debug output
	sr.PrettyPrint(2)
	...
	// set the authenticated user into request for further processing
	request.UserID = userdn
...
func (s *Storage) setUserinfo(ctx context.Context, userInfo oidc.UserInfoSetter, userID, clientID string, scopes []string) (err error) {
	...
	// Search for the given username
	searchRequest := ldap.NewSearchRequest(
		userID, // we store DN in userID but maybe should be replaced with proper search
		ldap.ScopeWholeSubtree, ldap.NeverDerefAliases, 0, 0, false,
		"(objectClass=*)",
		nil,
		nil,
	...
	for _, scope := range scopes {
		...
		case oidc.ScopeOpenID:
			userInfo.SetSubject(user.DN)
		case oidc.ScopeEmail:
			userInfo.SetEmail(user.GetAttributeValue("mail"), true)
		case oidc.ScopeProfile:
			userInfo.SetGivenName(user.GetAttributeValue("givenName"))
			userInfo.SetName(user.GetAttributeValue("fullName"))
		case CustomScope:

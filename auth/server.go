...
func SetupServer(ctx context.Context, issuer string) *mux.Router {
	storage := storage.NewStorage()

	// this will allow us to use an issuer with http:// instead of https://
	os.Setenv(op.OidcDevMode, "true")

	// the OpenID Provider requires a 32-byte key for encryption
	// this is just temporary and should be replaced
